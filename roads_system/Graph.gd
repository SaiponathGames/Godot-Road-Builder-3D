...
class Segment:
	var start_position: Point
	var end_position: Point
...
func connect_points(from_id, to_id) -> Segment:
	...
	connections[[from_id, to_id]] = Segment.new(get_point(from_id), get_point(to_id))
...
func are_points_connected(from_id, to_id):
	...
	if not to_id in points.keys():
		...
		return false
	return connections.has([from_id, to_id])
